...
class InferenceService:
    ...
    def load(self) -> None:
        ...
        if not model_path.exists():
            # Fallback to legacy model location if the notebook-produced checkpoint path is not present.
            legacy_path = Path(__file__).resolve().parents[3] / "models" / "cattle_breed_classifier_full_model.pth"
            if legacy_path.exists():
                logger.info(f"Using legacy fallback checkpoint: {legacy_path}")
                model_path = legacy_path
            else:
                logger.warning(f"Model file not found: {model_path}")
                logger.info(
                    "Service will start without a model. "
                    "Place a trained checkpoint at this path or set MODEL_PATH to a valid file."
                )
                return
